...
MSCC_Version = "Musicstream.cc v1.05"
...
class show_MSCC_Genre(MPScreen):
	...
	def __init__(self, session, baseUrl='http://musicstream.cc', url='/index.php?pwd=&d=0', ctitle="Musikalben", red_key="Exit"):
		self.genre_url = url
		self.ctitle = ctitle
	...
	def keyOK(self):
		...
		if musicfolder:
			self.session.open(show_MSCC_Genre, self.base_url, url, 'Album: %s' % album, "Zurück" )
		else:
